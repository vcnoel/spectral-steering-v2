...
% ============================================================
\begin{abstract}
Instruction tuned language models often defer to user opinions even when those opinions are factually wrong, a behavior known as sycophancy. While sycophancy is widespread across chat models, its weight space substrate has remained opaque, blocking principled mitigation. In this work, we show that sycophancy is mediated by the dominant singular subspace of a single MLP weight matrix in nine of ten open source models spanning 1B--8B parameters.

Specifically, for each affected model we find a single layer at which compressing the dominant spectral direction monotonically reduces the forced choice sycophancy rate, while amplifying it induces sycophancy on otherwise neutral inputs; aggressive compression at this layer reveals nonlinear capability trade offs that we analyze and operationalize as a layer geometry diagnostic. 

Leveraging this insight, we propose a closed form weight space intervention that requires no training, no contrastive data, and no inference time machinery, and that on Gemma-4-E2B-it produces a strict Pareto improvement: less sycophancy and more reasoning. Finally, we show that the per layer spectral profile partitions models into two storage classes, localized and distributed, predicting in advance whether single layer surgery will succeed. 

Our findings reframe the alignment tax from a fundamental training constraint to a measurable consequence of spectral geometry, and establish spectral diagnostics as a non behavioral audit primitive for instruction tuning regimes. Code can be accessed at \texttt{https://anonymous.4open.science/r/spectral-steering-v2-FABD/}.
\end{abstract}
...
\item \textbf{Localisation and bidirectional control.}
 In nine of ten tested instruction-tuned models, sycophancy is
 ...
 by up to $-76\pp$, amplification increases it by $+18\pp$, across 1B
 to 8B parameters and five architectural families (Llama, Gemma, Phi, Qwen, Mistral). We note (and
...
\item \textbf{Spectral diagnostic and storage class taxonomy.}
 ...
 \emph{localised, low rank} (Llama, Gemma, Phi, Qwen, SmolLM2 — surgery
 works) and \emph{distributed, high rank} (Mistral — surgery fails,
 SWA-linked). Gemma-2 represents an intermediate weakly-localised regime. Both
...
sycophancy under single-layer spectral steering ($N{\ge}100$ unless noted).
$^\S$Llama-3B at $\alpha{=}{-}1.5$ shows the headline A/B reduction; capability
is preserved only at $\alpha{=}{-}0.5$ ($-17\pp$ syco, $-32\pp$ GSM8K), see \Cref{sec:llama3b_entanglement}.
$^\natural$Gemma-4: dominant single-layer site from $N{=}30$ sweep; multi-layer Pareto in \Cref{sec:gemma_pareto}.
$^\ddagger$Mistral: inert at mild $\alpha$; collapses to $0\%$ at $\alpha{=}{-}1.0$ (manifold collapse, not genuine reduction); see \Cref{sec:mistral}.
$^\P$$N{=}50$ sweep, $\alpha{=}{-}1.5$; no capability benchmark.
$^\dagger$Qwen2.5-7B: validated at $N{=}100$ ($\alpha{=}{-}0.7$); GSM8K $52\%{\to}20\%$ ($-32\pp$). Aggressive sweep ($\alpha{=}{-}1.5$) gives $-76\pp$ but exceeds safe regime $|\alpha|\ll 1/\mathrm{SNR}{=}0.19$.}
\label{tab:summary}
...
Gemma-4-E2B$^\natural$ & 46 & L13 & 6.53 & $-1.0$ & 70\% & 40\% & $-30\pp$ \\
Gemma-2-2b & 26 & L8 & 4.57 & $-1.5$ & 90\%$^\P$ & 50\%$^\P$ & $-40\pp$ \\
Phi-4-mini & 32 & L10 & 4.59 & $-0.2$ & 86\% & 79\% [73,85] & $-7\pp$ \\
Mistral-7B-v0.3$^\ddagger$ & 32 & L31 & 2.69 & $-0.5$ & 98\% & 98\% & $0\pp$ \\
\midrule
Qwen2.5-3B$^\P$ & 36 & L6 & 6.32 & $-1.5$ & 82\% & 10\% & $-72\pp$ \\
Qwen2.5-7B$^\dagger$ & 28 & L16 & 5.15 & $-0.7$ & 90\% [84,95] & 43\% [34,52] & $-47\pp$ \\
SmolLM2-1.7B$^\P$ & 24 & L12 & 4.35 & $-1.5$ & 72\% & 16\% & $-56\pp$ \\
\bottomrule
\end{tabular}
\end{table}
...
A layer sweep of Gemma-4-E2B-it ($N{=}30$ per layer, $\alpha{=}{-}1.0$, 35
language-model layers) first establishes that the method works: the dominant
sycophancy site is L13 ($\snr{=}6.53$), which delivers $-30\pp$ reduction
at $\alpha{=}{-}1.0$. However, the adjacent site L12 ($\snr{=}5.83$)
undergoes full manifold collapse at the same dose ($0\%$ sycophancy,
$\mathrm{CI}{=}[0,0]$), and three further layers (L15, L29, L31) exhibit
\emph{sign reversal} ($+3$ to $+10\pp$), revealing that the compliance
direction runs opposite to the dominant spectral direction at those sites.

This fragility pattern is not accidental — it is predicted by the per-layer
SNR as a dosing guide. L13 ($\snr{=}6.53$) is near the boundary of the
safe operating regime:
\[
  |\alpha| \ll \frac{1}{\snr_\ell}.
\]
At $\snr{=}6.53$, the safe dose is $|\alpha| \ll 0.15$ — far smaller than
the $\alpha{=}{-}1.0$ applied in the layer sweep, placing L13 well into the
fragile regime. High-SNR layers carry heavy representational load; aggressive
$\alpha$ causes collapse because there is no spectral space to cut cleanly.

This suggests a different strategy: rather than forcing the dominant site at
full dose, combine \emph{low-SNR} layers, each operated within their
individually safe dose. The per-layer SNR profile identifies three such sites
in Gemma-4-E2B-it — L18 ($\snr{=}4.42$), L24 ($\snr{=}3.88$), and L33
($\snr{=}3.13$) — among the flattest spectral sites in the network. At low
SNR, spectral mass is distributed across many directions; even a tiny $\alpha$
produces a controlled perturbation that can thread between sycophancy and
reasoning subspaces.

We conduct a joint hyperparameter search over these three layers and
amplitudes $\alpha \in [-0.07, +0.10]$ (guided by the SNR profile;
\Cref{app:gemma_full}). All configurations are evaluated at
$N{=}1{,}000$ sycophancy items and $N{=}250$ GSM8K items across 5 seeds.
...
Both objectives improve simultaneously, each well outside the 95\% CI
of their respective baselines (sycophancy $[36,43]$; GSM8K $[30,43]$).
...
must incur capability cost. The Pareto improvement is not merely a property
of the \emph{layer} but of the \emph{interaction} between SNR and dose:
operating at low-SNR sites within the safe regime frees representational
capacity that is redirected towards mathematical reasoning rather than
...
Unlike the Llama, Gemma, Phi, and Qwen models, Mistral-7B-Instruct-v0.3 resists single-layer spectral surgery. Its spectral profile lacks a concentrated dominant mode: per-layer SNR remains below $3.5$ (near the Marchenko-Pastur noise floor), contrasting sharply with models like Llama-3.1-8B ($\snr{=}5.25$) or Qwen2.5-7B ($\snr{=}5.15$ at L16).

The failure mode is binary with no intermediate regime. At $\alpha{=}{-}0.5$ on the highest-SNR layer (L31), sycophancy is completely inert ($98\%{\to}98\%$, zero change). At $\alpha{=}{-}1.0$ on the same layer, the model undergoes full manifold collapse ($98\%{\to}0\%$, $\mathrm{CI}{=}[0,0]$) — not a genuine reduction but an inability to produce coherent A/B outputs. There is no dose that produces targeted behavioural change.

Decomposing $\Delta W = W_\mathrm{instruct} - W_\mathrm{base}$ reveals why: RLHF wrote the compliance update directly into the primary spectral direction ($\cos\theta{=}0.812$ at L31). Any dose strong enough to suppress compliance also destroys core representations — the two are inseparable. By contrast, in Llama-3.2-3B L7, $\cos\theta{=}0.025$, placing the compliance update near-orthogonally to the dominant direction and enabling the clean $-50\pp$ reduction we observe (full contrastive analysis in \Cref{app:mistral_multilayer}).

\paragraph{Why Mistral stores compliance differently.}
A plausible architectural cause is Mistral's \emph{Sliding Window Attention} (SWA), which restricts each attention layer to a local context window of 4096 tokens throughout the entire network. With attention bandwidth constrained to local context, cross-positional information — including compliance signals such as ``is the user asserting something I should agree with?'' — must be routed through the MLP sublayers. The $W_\mathrm{down}$ matrices in Mistral therefore carry more diverse computational roles than in full-attention models. When RLHF trains for compliance, it finds no orthogonal subspace: it is forced to write the compliance update into the existing dominant spectral directions ($\cos\theta{=}0.812$).

This predicts a gradient across attention architectures, which our expanded results support: Mistral (all-SWA, all layers restricted) $\rightarrow$ distributed, surgery fails; Gemma-2 (alternating SWA and global attention) $\rightarrow$ weakly localised, $-40\pp$ best single-layer; full-attention models (Llama, Qwen, SmolLM2) $\rightarrow$ strongly localised, $-50$ to $-76\pp$. The ΔW fingerprint ($\cos(\Delta W_{\mathbf{u}_1}, W_{\mathbf{u}_1})$) provides a fast spectral test of which regime any model falls into, requiring no behavioural evaluation.

This establishes a taxonomy recoverable entirely from the SVD profile of $W_\mathrm{down}$:

\begin{itemize}[leftmargin=1.4em,itemsep=1pt,topsep=2pt]
\item \textbf{Localised, low-rank} (Llama, Gemma-4, Phi, Qwen, SmolLM2 — full or mostly-full attention): Sharp dominant mode; surgery is highly effective.
\item \textbf{Weakly localised} (Gemma-2 — alternating SWA/global): Surgery works but with reduced potency ($-40\pp$ vs.\ $-76\pp$).
\item \textbf{Distributed, high-rank} (Mistral — all-SWA): Compliance co-located with dominant spectral directions; inert at mild dose, collapse at strong dose.
\end{itemize}

\begin{findingbox}
\textbf{Finding 5 (Storage-Class Taxonomy).}
Instruction-tuned LLMs partition into spectral storage classes recoverable from the per-layer SNR profile and $\Delta W$ fingerprint: \emph{localised} (surgery works, $\cos(\Delta W_{\mathbf{u}_1}, W_{\mathbf{u}_1}) \approx 0$, 9/10 models); \emph{distributed} (surgery fails, $\cos(\Delta W_{\mathbf{u}_1}, W_{\mathbf{u}_1}) \approx 0.81$, Mistral-7B). The SWA architecture correlates with the distributed class, offering a causal hypothesis linking attention mechanism design to the viability of weight-space behavioural surgery.
\end{findingbox}
...
\subsection{Matrix specificity: why \texttt{down\_proj}?}
\label{sec:matrix_ablation}

A natural ablation is to apply the same spectral rescaling to \texttt{up\_proj} or \texttt{gate\_proj} at the same layer and measure the effect. We run this comparison on Llama-3.2-3B-Instruct at the auto-selected 75th-percentile depth layer (L21, $\snr_\mathrm{down}{=}3.03$, $\snr_\mathrm{gate}{=}5.97$, $N{=}50$ syco, $N{=}30$ GSM8K).

The result reveals a double specificity. At L21, \texttt{down\_proj} is spectrally quiet ($\snr{=}3.03$) and behaviorally inert: sycophancy barely moves ($\Delta{\approx}0\pp$) and GSM8K is unaffected at mild $\alpha$. The \texttt{gate\_proj} at the same layer has high SNR ($5.97$) and does reduce sycophancy (${-}34$--${-}56\pp$ at $\alpha{=}{-}0.5$ to $-1.5$) — but destroys all reasoning capability simultaneously (GSM8K $70\%{\to}0\%$ at \emph{every} dose). The \texttt{up\_proj} shows an intermediate pattern: modest sycophancy reduction (${-}18$--${-}30\pp$) accompanied by progressive capability damage.

This confirms that our method's targeted effect requires \emph{both} the correct matrix (\texttt{down\_proj}) and the correct layer (the dominant spectral site identified by the SNR prefilter). \texttt{gate\_proj} surgery is indiscriminate: its high SNR at L21 is entangled with core computation, so any dose strong enough to suppress compliance also suppresses reasoning. \texttt{down\_proj} at the dominant layer (L7 for Llama-3.2-3B) is different because RLHF specifically inscribed compliance into a direction that is \emph{low-rank and separable} from the main computational load — a property revealed by the ΔW analysis ($\cos\theta{=}0.025$) and exploited by our method.
...
including sycophancy and modulate them at deployment. All such methods require contrastive prompt data and
...
% ============================================================
% Drop-in replacement for the \begin{thebibliography} ... \end{thebibliography}
% block in main_v5.tex.  Keys match every \cite{} in the body.
% ============================================================
...
%%% Content fix: year 2005→2006; key kept as-is to match body \cite
\bibitem[Baik and Silverstein(2006)]{baik2005phaseTransition}
...
%%% Content fix: year 2026→2025 (original submission); key kept as-is
\bibitem[Fierro and Roger(2025)]{fierroWeightArithmetic2026}
Fierro, C. and Roger, F.
\newblock Steering language models with weight arithmetic.
\newblock \emph{arXiv:2511.05408}, 2025.
...
%%% Content fix: no Gemma 4 report exists; using Gemma 3 (arXiv:2503.19786)
\bibitem[Gemma Team(2025)]{gemmateamGemma4Technical2025}
Gemma Team.
\newblock Gemma 3 technical report.
\newblock \emph{arXiv:2503.19786}, 2025.

%%% Content fix: author spelling Hendrycks (not Hendryks); year 2020
\bibitem[Hendrycks et~al.(2020)]{hendryksScalingInstructionFinetunedLanguage2022}
...
\bibitem[Marks et~al.(2025)]{marksSparseFeatureCircuits2024}
...
%%% Content fix: venue ICML 2025→arXiv 2026 (paper posted Jan 2026)
\bibitem[No\"el et~al.(2026)]{noelGeometryReasonSpectral2025}
No\"el, V. et~al.
\newblock Geometry of reason: Spectral signatures of valid mathematical
 reasoning.
\newblock \emph{arXiv:2601.00791}, 2026.
...
%%% Content fix: corrected author list (29 real authors)
...
Sharkey, L., Chughtai, B., Batson, J., Lindsey, J., Wu, J.,
 ...
 Bloom, J., Biderman, S., Garriga-Alonso, A., Conmy, A., Nanda, N.,
 Rumbelow, J., Wattenberg, M., Schoots, N., Miller, J.,
 Michaud, E.~J., Casper, S., Tegmark, M., Saunders, W., Bau, D.,
 Todd, E., Geiger, A., Geva, M., Hoogland, J., Murfet, D.,
 and McGrath, T.
...
%%% Content fix: added missing co-author Zimo Wang
\bibitem[Sun et~al.(2026)]{sunSteer2Edit2026}
Sun, C.~E., Yan, G., Wang, Z., and Weng, T.
...
\textbf{Mild-$\alpha$ open-ended probe.} We repeated the same $N{=}25$ essay experiment at the capability-preserving operating point ($\alpha{=}{-}0.5$, L7). The judge again reports $100\%$ sycophancy for both baseline and steered conditions — but inspection of the actual responses reveals the judge's limitation: \emph{both} conditions produce corrections of the embedded factual error (``Your paragraph has a small mistake: ...''). The 1B judge cannot distinguish a politely framed correction (``Good start, but the correct formula is...'') from sycophantic validation. This exposes a deeper issue with the task: in \emph{open-ended} format, Llama-3.2-3B is not strongly sycophantic even at baseline — it hedges with corrections framed politely. The A/B forced-choice format is more diagnostic precisely because it removes the hedging option, forcing a binary commitment to the user's (wrong) answer. The two formats therefore measure related but distinct phenomena: the A/B format captures \emph{conformity under binary pressure}; open-ended evaluation captures \emph{willingness to confront in free text}. Our spectral intervention reduces the former; its effect on the latter requires a more capable judge than the 1B model used here.
