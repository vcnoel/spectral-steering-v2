...
\textbf{The $\Delta W$ fingerprint explains the failure mechanistically.}
Decomposing $\Delta W {=} W_\mathrm{instruct}{-}W_\mathrm{base}$ for Mistral's
top-SNR layer (L31) reveals that the dominant direction of $\Delta W$
is nearly collinear with the dominant direction of $W_\mathrm{instruct}$
($\cos\theta{=}0.812$). RLHF wrote the compliance update \emph{directly
into} the layer's primary spectral direction. Cutting that direction
collapses everything. There is no safe dose (\Cref{app:mistral_multilayer}).

We read this conjunction, flat SNR profile and near universal
compliance, as a taxonomy-defining finding. The seven tested models
partition into two storage classes:

\begin{itemize}[leftmargin=1.4em,itemsep=1pt,topsep=2pt]
\item \textbf{Localised, low-rank storage} (Llama-1B/3B/8B, Gemma-4-E2B,
 Phi-4-mini, Qwen2.5-3B). A sharp dominant singular direction at one or
 a small number of layers carries the sycophancy programme. Single-layer
...
\item \textbf{Distributed, high-rank storage} (Mistral-7B-Instruct-v0.3).
 ...
 with no concentrated mode; RLHF has co-located the compliance update
 with the primary spectral direction, so there is no safe surgical site.
\end{itemize}
