...
sycophancy under single-layer spectral steering ($N{\ge}100$,
5 seeds, 95\% CI unless noted). $^\ddagger$Mistral: surgery fails — inert at mild $\alpha$, collapses at $\alpha{\ge}1.0$; see \Cref{sec:mistral}.
$^\S$Llama-3B at $\alpha{=}{-}1.5$ shows the headline A/B
reduction; capability is preserved only at $\alpha{=}{-}0.5$ ($-17\pp$
sycophancy with $-32\pp$ GSM8K), see \Cref{sec:llama3b_entanglement}.
$^\natural$Gemma-4: dominant single-layer site from $N{=}30$ sweep ($\alpha{=}{-}1.0$); single-layer aggressive, multi-layer Pareto in \Cref{sec:gemma_pareto}.
$^\P$Qwen2.5-3B-Instruct: $N{=}50$ sweep only; no capability benchmark yet. L1 has the highest observed SNR of any model tested ($\mathrm{SNR}{=}20.84$).}
\label{tab:summary}
...
Mistral-7B-v0.3$^\ddagger$ & 32 & L31 & 2.69 & -- & 98\% & -- & not reducible \\
\midrule
